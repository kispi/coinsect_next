...
export default function CPosition({ position: initialPos }: Props) {
  ...
  const isDanger =
    initialPos.liqPrice &&
    unrealizedPnl < 0 &&
    Math.abs(initialPos.liqPrice - currentMarkPrice) / currentMarkPrice < 0.005
  ...
  return (
    ...
    >
      ...
      <div className="relative w-20 flex-shrink-0 cursor-pointer hover:opacity-50 transition-opacity bg-black">
        <img
          src={initialPos.image || '/images/default-avatar.png'}
          alt={initialPos.name}
        ...
        <div className="absolute top-0 left-0 right-0 p-1 bg-gradient-to-b from-black/80 to-transparent text-white flex justify-between items-start text-[10px]">
          <div className="flex items-center bg-black/50 border border-white/10 rounded-full px-1">
            <div
              className={`w-1.5 h-1.5 rounded-full mr-1 ${initialPos.onAir ? 'bg-green-500' : 'bg-red-500'}`}
            />
      ...
      <div className="flex-1 p-2 flex flex-col gap-2">
        <div className="flex gap-2 text-[11px]">
          ...
          <div className="flex-1 flex flex-col items-start">
            ...
            <div
              className={`font-mono font-medium ${isLong ? 'text-emerald-600 dark:text-emerald-400' : isShort ? 'text-rose-600 dark:text-rose-400' : 'text-text-base'}`}
            >
          ...
          <div className="flex-1 flex flex-col items-end">
            ...
            <div
              className={`font-mono font-medium ${unrealizedPnl > 0 ? 'text-emerald-600 dark:text-emerald-400' : unrealizedPnl < 0 ? 'text-rose-600 dark:text-rose-400' : 'text-text-base'}`}
            >
        ...
        <div className="flex gap-2 text-[11px]">
          ...
          <div className="flex-1 flex flex-col items-start">
            ...
            <div
              className={`font-mono text-text-base transition-colors duration-100 ${isDanger ? 'text-rose-500' : ''}`}
            >
