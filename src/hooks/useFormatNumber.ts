...
export function useFormatNumber() {
  ...
  const getConversionRatio = useCallback(({ baseCurrency, noConversion }: { baseCurrency: string, noConversion?: boolean }) => {
    if (
      noConversion ||
  ...
  const formatPrice = useCallback(({ price, baseCurrency, fracs, noConversion }: FormatPriceOptions) => {
  ...
  const formatCap = useCallback(({ cap, baseCurrency, numKorUnits = 2 }: FormatCapOptions) => {
    ...
    if (settings.locale === 'en') {
      ...
      if (absConverted / Math.pow(10, 12) >= 1) return `${Math.round(converted / Math.pow(10, 12) * 10000) / 10000}T`;
      if (absConverted / Math.pow(10, 9) >= 1) return `${Math.round(converted / Math.pow(10, 9) * 10000) / 10000}B`;
      if (absConverted / Math.pow(10, 6) >= 1) return `${Math.round(converted / Math.pow(10, 6) * 10000) / 10000}M`;
      if (absConverted / Math.pow(10, 3) >= 1) return `${Math.round(converted / Math.pow(10, 3) * 10000) / 10000}K`;
