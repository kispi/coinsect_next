...
1. **타임스탬프**: 파일명 및 문서 내의 모든 타임스탬프는 예외 없이 **24시간 형식(HH:mm:ss)**으로 작성합니다. (AM/PM 사용 금지)
2. **제목**: `# TASK: [작업 제목]`
...
- **모든 상호작용 가능한 요소(버튼, 아이콘, 프로필 이미지 등)는 항상 `cursor: pointer`를 가져야 함.**
...
## 6. Vue 코드를 기반으로 마이그레이션을 진행하게 되는데, 원래 코드와 완전히 논리적으로 동일할 필요가 없으며 비슷하게 동작하면 됨. 원래 코드가 지닌 문제점이 있거나, 그 방식이 구식이라면 **현재 업계의 best practice**인 방식으로 개선해서 마이그레이션 필요.

ex1) 아이콘이 원래 코드베이스의 그것과 완전히 동일한게 없다면 lucide-react에서 가장 비슷한 것을 사용.
ex2) 컴포넌트에서 직접 API를 호출하고 있다면 Tanstack을 이용한 hook 형태로 변경.