...
- **임시 파일 정리**: 작업 중간에 생성한 스크립트(.py), 결과 로그(.txt) 등의 임시 파일들은 모든 작업이 완료된 후 반드시 삭제하여 코드베이스를 깨끗하게 유지합니다.
...
1. **제목**: `# TASK: [작업 제목]`
2. **섹션 1**: `## WHAT` (작업 결과 요약)
3. **섹션 2**: `## WHY` (비즈니스/기술적 배경 및 근거)
4. **섹션 3**: `## HOW & WHY` (아키텍처 선택, 라이브러리 활용 등)
5. **섹션 4**: `## RESULT` (기능적 변화, 성능 향상 등)
